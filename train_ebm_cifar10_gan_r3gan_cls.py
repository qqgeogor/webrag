...
class EnergyNet(nn.Module):
    def __init__(self, img_channels=3, hidden_dim=64, num_classes=10, embedding_dim=100):
        super().__init__()
    ...
    def forward(self, x, labels):
        ...
        label_embedding = self.embedding(labels)  # [B, embedding_dim]
        # print('label_embedding',label_embedding.shape)
        # print('x',x.shape)
        label_plane = label_embedding.view(batch_size, 1, 1, 1).expand(-1, 1, 32, 32)
        # Concatenate image and label channel
        x = torch.cat([x, label_plane], dim=1)
...
class Generator(nn.Module):
    def __init__(self, latent_dim=100, hidden_dim=64, num_classes=10, embedding_dim=100):
        super().__init__()
...
def train_ebm_gan(args):
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, (real_samples, labels) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):  # Train discriminator more frequently
                ...
                # Pass labels to discriminator
                real_energy = discriminator(real_samples, labels)
                fake_energy = discriminator(fake_samples, labels)
...
def save_gan_samples(generator, discriminator, epoch, output_dir, device, n_samples=36):
    ...
    with torch.no_grad():
        ...
        # Create labels (all same class since we're training on class 1)
        labels = torch.ones(n_samples, dtype=torch.long, device=device)
        fake_samples = generator(z, labels)
        ...
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, f'gan_samples_epoch_{epoch}_1.png'))
        ...
        plt.imshow(grid.cpu().permute(1, 2, 0))
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, f'gan_samples_epoch_{epoch}_0.png'))
        # Changed 'range' to 'value_range'
        labels = torch.ones(n_samples, dtype=torch.long, device=device)*2
        fake_samples = generator(z, labels)
        grid = make_grid(fake_samples, nrow=6, normalize=True, value_range=(-1, 1))
        plt.figure(figsize=(10, 10))
        plt.imshow(grid.cpu().permute(1, 2, 0))
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, f'gan_samples_epoch_{epoch}_2.png'))

        
        # Changed 'range' to 'value_range'
        labels = torch.ones(n_samples, dtype=torch.long, device=device)*3
        fake_samples = generator(z, labels)
        grid = make_grid(fake_samples, nrow=6, normalize=True, value_range=(-1, 1))
        plt.figure(figsize=(10, 10))
        plt.imshow(grid.cpu().permute(1, 2, 0))
