...
class MaskedAutoencoderViT(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_chans=3,
        ...
        self.project_latent = nn.Sequential(
            # Initial projection
            nn.Linear(decoder_embed_dim*2, decoder_embed_dim * num_patches),
            # Reshape layer instead of lambda
...
class Generator(nn.Module):
    def __init__(self, latent_dim=100, hidden_dim=64, img_size=32):
        ...
        self.project = nn.Sequential(
            nn.Linear(latent_dim*2, hidden_dim * 8 * 4 * 4),
            nn.LeakyReLU(0.2),
        )
...
def train_ebm_gan(args):
    ...
    generator = Generator(latent_dim=args.embed_dim, hidden_dim=64, img_size=args.img_size).to(device)
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, (real_samples, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):  # Train discriminator more frequently
                ...
                # c_fake = discriminator.forward_feature(aug_samples.detach()).squeeze()
                ...
                loss_tcr = -tcr_loss(c_real,c_fake)*1e-2
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='F:/output/tiny_imagenet_ctrl_vit_dino')
