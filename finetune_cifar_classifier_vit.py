...
class CifarClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.freeze_backbone:
            with torch.no_grad():
                features = self.backbone.forward_feature(x)
        else:
            features = self.backbone.forward_feature(x)
...
def finetune(args):
    ...
    backbone = MaskedAutoencoderViT(img_size=32, patch_size=4, in_chans=3,
                 embed_dim=192, depth=6, num_heads=3,
                 decoder_embed_dim=192, decoder_depth=4, decoder_num_heads=3,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, use_checkpoint=True).to(device)
